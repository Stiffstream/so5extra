...
{
public :
	struct get_value final : public so_5::message_t
	{
		const int m_key;

		get_value(int key) : m_key(key) {}
	};

	a_service_provider_t( context_t ctx, const so_5::mbox_t & mbox )
		:	so_5::agent_t( std::move(ctx) )
	{
		so_set_delivery_filter(mbox,
				[](const get_value & v){ return v.m_key != 0; });
...
public :
	...
	so_evt_start() override
	{
		...
		{
			ensure_or_die(
				so_5::rc_no_svc_handlers == x.error_code(),
				"rc_no_svc_handlers is expected as error code, got: " +
