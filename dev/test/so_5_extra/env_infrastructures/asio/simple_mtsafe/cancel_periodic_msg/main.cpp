...
const clock_type::duration finish_pause = std::chrono::milliseconds(500);

class a_test_case_t final : public so_5::agent_t
{
	test_data_t & m_data;

	int m_hello_received{ 0 };
	static constexpr int max_expected_hellos = 3;
...
private :
	...
	{
		++m_hello_received;
		if( max_expected_hellos == m_hello_received )
			m_hello_timer.release();
		else if( max_expected_hellos < m_hello_received )
			throw std::runtime_error( "hello signal received to many times: " +
					std::to_string(m_hello_received) );
	}
...
TEST_CASE( "cancel periodic signal" )
{
	run_with_time_limit( [] {
