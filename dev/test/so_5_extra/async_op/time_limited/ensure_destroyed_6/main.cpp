		...
		so_evt_start() override
			{
				so_default_state()
					.event( [this]( mhood_t<cancel> ) {
							ensure_or_die( 1 == test_op_data_t::live_items(),
									"There should be one live op_data instances" );

							m_cp.cancel();

							ensure_or_die(
									asyncop::status_t::cancelled == m_cp.status(),
									"status_t::cancelled is expected" );

							ensure_or_die( 1 == test_op_data_t::live_items(),
									"There still should be one live op_data instances" );

							m_cp.cleanup();

							ensure_or_die(
									asyncop::status_t::unknown_moved_away == m_cp.status(),
									"status_t::unknown_moved_away is expected" );
