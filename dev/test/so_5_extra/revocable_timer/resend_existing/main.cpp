...
class test_case_t final : public so_5::agent_t
	{
		...
		const so_5::state_t st_first{ this };
		const so_5::state_t st_second{ this };
	...
	public :
		test_case_t(
			...
			{
				st_first
					.event( &test_case_t::on_receive_first );

				st_second
					.event( &test_case_t::on_receive_second )
		...
		so_evt_start() override
			{
				...
				Sender::send_first( *this );
			}


	private :
		void
		on_receive_first( mhood_t<Message> cmd )
			{
				this >>= st_second;

				auto timer_id = Sender::send_second( *this, std::move(cmd) );
		...
		void
		on_receive_second( mhood_t<Message> )
			{
...
struct first_msg_sender_t
	{
		static void
		send_first( const so_5::agent_t & to )
			{
				so_5::send<Message>( to, 0, "Hello!" );
			}
	};

template< typename Message >
struct send_periodic_env_mbox_t : public first_msg_sender_t< Message >
	{
		static auto
...
template< typename Message >
struct send_delayed_env_mbox_t : public first_msg_sender_t< Message >
	{
		static auto
		send_second( const so_5::agent_t & to, so_5::mhood_t<Message> cmd)
			{
...
template< typename Message >
struct send_periodic_agent_t : public first_msg_sender_t< Message >
	{
		static auto
		send_second( const so_5::agent_t & to, so_5::mhood_t<Message> cmd)
			{
...
template< typename Message >
struct send_delayed_agent_t : public first_msg_sender_t< Message >
	{
		static auto
		send_second( const so_5::agent_t & to, so_5::mhood_t<Message> cmd)
			{
...
template< typename Message >
struct first_signal_sender_t
	{
		static void
		send_first( const so_5::agent_t & to )
			{
				so_5::send<Message>( to );
			}
...
struct send_periodic_signal_env_mbox_t
	:	public first_signal_sender_t< Message >
	{
		static auto
		send_second( const so_5::agent_t & to, so_5::mhood_t<Message> cmd )
			{
